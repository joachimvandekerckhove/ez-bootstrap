...
Other members of the EZ class include the EZ diffusion model (\citeNP{WagenmakersEtAl2007}), the EZ circular diffusion model \cite{QarehdaghiRad2024}, and some simple multinomial processing tree models with analytical solutions. \claude{These models are discussed in detail below.}
...
In applied contexts, we often constrain parameters across conditions. These constraints can be expressed using design matrices. For example, \claude{for diffusion models,} if drift rates, \claude{$\drift$,} vary linearly with a stimulus property $x_i$, we can write:
...
\end{eqnarray}
\claude{where $\text{sign}(x)$ denotes the sign function, which returns $+1$ if $x \geq 0$ and $-1$ if $x < 0$. In this context, it determines whether drift is positive (when accuracy $\geq 0.5$) or negative (when accuracy $< 0.5$).}
...
\claude{A Python implementation of the EZ-DDM is also available as part of the larger EZAS package (\url{https://github.com/joachimvandekerckhove/ezas}), which uses the same method but is not as fast as the C implementation.}
