...
\jv{You can leave notes with \texttt{\textbackslash{}elf\{ <note text> \}.}}
...
Other members of the EZ class include the EZ diffusion model (\citeNP{WagenmakersEtAl2007}; discussed in detail below), the EZ circular diffusion model \cite{QarehdaghiRad2024}, and some simple multinomial processing tree models with analytical solutions. 
\elf{ 'discussed in detail below' : all of these are discussed below, not just EZ diffusion, right?}
...
The key advantage of this approach is computational efficiency. Rather than resampling raw data and re-estimating parameters for each bootstrap sample---which would require expensive optimization or simulation \elf{e.g., citation (if applicable)}---we resample the summary statistics from their known distributions and apply the analytical transformation. \elf{i.e., Algorithm 1?} This eliminates the computational bottleneck while preserving the statistical properties of the bootstrap.
...
In applied contexts, we often constrain parameters across conditions. These constraints can be expressed using design matrices. For example, \elf{for diffusion models,} if drift rates, \elf{$\drift$}, vary linearly with a stimulus property $x_i$, we can write:
$$\drift_i = \beta_0 + \beta_1 x_i$$

This generalizes, so that parameter constraints can be expressed as:
$$\Delta = X \times B$$
\elf{Assuming I am following correctly, $B$ is already defined in Algorithm 1 for number of bootstrap samples, right?}
...
\elf{What does 'sign' mean in the above equation for drift?}
...
\elf{I think there is an extra $^2$ above: $s_{i}^{22}$}
...
\elf{Curiosity: Is there representation for a choice selection of 'New' when 'Old' is A or B? i.e., $N_{A}, N_{B}$}
...
\begin{itemize}
  \item 
  \textbf{Model scope:} The method is limited to the class of EZ models and their design matrix extensions. While identifiable models will typically \textit{have} both the forward and inverse equations, the equations are often not known or are mathematically inconvenient.
  \item
...
\jv{Mention EZAS package here too.}
