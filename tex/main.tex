...
The circular mean and variance are:
\begin{eqnarray}
\MCA{} &=& \driftangle \nonumber\\
...
% \footnote{A standard approximation for solving $R = I_1(k)/I_0(k)$ for $k$ is the piecewise estimator of Fisher \cite[p.~88]{fisher1993circular}: $k \approx \begin{cases} 2R + R^{3} + \tfrac{5}{6}R^{5}, & R < 0.53,\\ -0.4 + 1.39R + \dfrac{0.43}{1-R}, & 0.53 \le R < 0.85,\\ \dfrac{1}{R^{3} - 4R^{2} + 3R}, & R \ge 0.85.\end{cases}$}
\begin{eqnarray}
\hat{\driftangle} &=& \MCA{} \nonumber\\
...
\end{equation}
Consequently, $\widehat{\VCA{}} = 1 - R$ is also asymptotically normal with the same variance. The circular variance is insensitive to outliers in angle (since angles wrap on the circle), but becomes unstable when $R$ is near zero.

For mean response time ($\MRT{}$), the central limit theorem applies:
\begin{equation}
\widehat{\MRT{}} \sim \mathcal{N}\left(E[T], \frac{\operatorname{Var}(T)}{N}\right) \nonumber
\end{equation}
where $E[T]$ and $\operatorname{Var}(T)$ are given by the forward equations. Mean RT is robust for large $N$ but sensitive to RT contaminants.

For response time variance ($\VRT{}$), the sample variance is asymptotically normal but with slower convergence:
\begin{equation}
\widehat{\VRT{}} \sim \mathcal{N}\left(\operatorname{Var}(T), \frac{\mu_4 - \operatorname{Var}(T)^2}{N}\right) \nonumber
\end{equation}
where $\mu_4$ is the fourth central moment of the RT distribution. This statistic is the most contamination-sensitive in EZ-CDM, as a small number of extreme RTs can cause large upward shifts (see \citeNP{chávez_de_la_peña_etal:preprint:hierarchical}, for a possible treatment using robust summary statistics).
