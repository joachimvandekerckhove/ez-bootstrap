...
% This replaces the ≈ character (U+2248) with LaTeX math mode \approx
% Note: The character must be specified exactly as it appears in the file
...
% \textit{Results.}
We evaluated coverage at the 95\% confidence level across 10,000 simulation repetitions. The EZ bootstrap method achieved excellent calibration. Coverage rates were 95.1\%, 94.4\%, and 96.0\% for boundary, drift, and non-decision time, respectively---all within expected sampling variation of the nominal 95\% level. When considering all three parameters simultaneously, the coverage rate was 87.1\%, closely matching the expected value ($0.95^3 \approx 0.857$).

\subsection{Study 2: Design matrix extension}
...
% \textit{Results.}
The EZ bootstrap method maintained excellent calibration even with the added complexity of the design matrix structure. For boundary weights, coverage rates were 95.4\%, 94.8\%, and 96.6\% across the three weights. For drift weights, coverage rates were 94.5\%, 94.2\%, and 97.8\%. For non-decision time weights, coverage rates were 96.2\%, 94.5\%, and 95.3\%. All coverage rates were within expected sampling variation of the nominal 95\% level.

\subsection{Computational performance}

The EZ bootstrap method is exceptionally computationally efficient. For Study~1, the method required a mean of only 0.22~ms per estimation (SD = 0.06~ms), enabling rapid uncertainty quantification. For Study~2, with the added complexity of the design matrix, the method required a mean of 1.33~ms per estimation (SD = 0.15~ms). These computation times demonstrate that the method makes real-time\footnote{We have a strict definition of ``real-time'' as less than the duration of one frame in a typical experimental setup -- usually at least 1/120th of a second or 8.33~ms.} uncertainty quantification feasible where traditional methods would be prohibitively slow.


\section{Other models}

The EZ bootstrap method generalizes to any model in the EZ class -- that is, any model with analytical forward and inverse equations mapping parameters to summary statistics. Here we briefly describe two additional examples: the EZ circular diffusion model (EZ-CDM) and the one-high-threshold source-monitoring MPT model.

\subsection{EZ circular diffusion model}

The circular diffusion model \cite{Smith2016} extends the diffusion model to circular decision spaces, where evidence accumulation occurs in a two-dimensional disk rather than along a one-dimensional line. The model has four parameters: drift angle $\driftangle$, drift magnitude $\driftmag$, decision criterion (boundary radius) $\boundaryradius$, and non-decision time $\ndtcdm$. The EZ version of the circular diffusion model (EZ-CDM; \citeNP{QarehdaghiRad2024}) is a synthetic likelihood model that uses four summary statistics: circular mean of choice angles ($\MCA{}$), circular variance of choice angles ($\VCA{}$), mean response time ($\MRT{}$), and response time variance ($\VRT{}$).

The forward equations $f_\text{CDM}: \left(\driftangle, \driftmag, \boundaryradius, \ndtcdm\right) \mapsto \left(\MCA{}, \VCA{}, \MRT{}, \VRT{}\right)$ map parameters to summary statistics. The choice-angle distribution is von Mises with mean $\mu = \driftangle$ and concentration $\kappa = \boundaryradius\driftmag$. The circular mean and variance are:
\begin{eqnarray}
\MCA{} &=& \driftangle \nonumber\\
R &=& \frac{I_1(\kappa)}{I_0(\kappa)}, \quad \VCA{} = 1 - R \nonumber
\end{eqnarray}
where $I_0$ and $I_1$ are modified Bessel functions of the first kind. The response time moments are:
\begin{eqnarray}
\MRT{} &=& \ndtcdm + \frac{\boundaryradius}{\driftmag} \frac{I_1(\boundaryradius\driftmag)}{I_0(\boundaryradius\driftmag)} \nonumber\\
\VRT{} &=& \frac{\boundaryradius^2}{\driftmag^2} \left( \frac{I_1(\boundaryradius\driftmag)}{I_0(\boundaryradius\driftmag)} \right)^2 + \frac{2\boundaryradius}{\driftmag^3} \frac{I_1(\boundaryradius\driftmag)}{I_0(\boundaryradius\driftmag)} - \boundaryradius^2 \nonumber
\end{eqnarray}
...
\hat{\driftmag}^4 &=& \frac{1}{\VRT{}}\left( \kappa^2 R^2 + 2\kappa R - \kappa^2 \right), \quad \hat{\boundaryradius} = \frac{\kappa}{\hat{\driftmag}} \nonumber\\
\hat{\ndtcdm} &=& \MRT{} - \frac{\hat{\boundaryradius}}{\hat{\driftmag}}R \nonumber
\end{eqnarray}

% \textit{Sampling distributions.}
The sampling distributions of the four summary statistics determine the variability and robustness of EZ-CDM parameter estimates. Under the simple CDM, choice angles follow a von Mises distribution with concentration $\kappa = \boundaryradius\driftmag$, and response times follow the first-passage-time distribution of a two-dimensional Bessel process with drift.

For the circular mean of choice angles (MCA), the angular error $\widehat{\driftangle} - \driftangle$ is approximately normally distributed for large $N$:
\begin{equation}
\widehat{\driftangle} - \driftangle \sim \mathcal{N}\left(0, \frac{1}{N\kappa A_1(\kappa)}\right) \nonumber
\end{equation}
where $A_1(\kappa) = I_1(\kappa)/I_0(\kappa)$. The variance decreases rapidly with concentration $\kappa$, making estimation most difficult when drift magnitude or boundary radius are small.

For the circular variance (VCA), the mean resultant length $R = \sqrt{\bar{c}^2 + \bar{s}^2}$ is approximately normal for moderate-to-large $N$, with mean $A_1(\kappa)$ and variance:
\begin{equation}
\operatorname{Var}(R) \approx \frac{1 - A_1(\kappa)^2}{N} \nonumber
\end{equation}
Consequently, $\widehat{\mathrm{VCA}} = 1 - R$ is also asymptotically normal with the same variance. The circular variance is insensitive to outliers in angle (since angles wrap on the circle), but becomes unstable when $R$ is near zero.

For mean response time (MRT), the central limit theorem applies:
\begin{equation}
\widehat{\mathrm{MRT}} \sim \mathcal{N}\left(E[T], \frac{\operatorname{Var}(T)}{N}\right) \nonumber
\end{equation}
where $E[T]$ and $\operatorname{Var}(T)$ are given by the forward equations. Mean RT is robust for large $N$ but highly sensitive to RT contaminants, with bias growing linearly in the contamination proportion.

For response time variance (VRT), the sample variance is asymptotically normal but with slower convergence:
\begin{equation}
\widehat{\mathrm{VRT}} \sim \mathcal{N}\left(\operatorname{Var}(T), \frac{\mu_4 - \operatorname{Var}(T)^2}{N}\right) \nonumber
\end{equation}
where $\mu_4$ is the fourth central moment of the RT distribution. This statistic is the most contamination-sensitive in EZ-CDM, as a small number of extreme RTs can cause large upward shifts.
...
\end{eqnarray}
These linear relationships allow closed-form inversion, illustrating one of the simplest nontrivial MPTs for which both forward and inverse transformations are available in closed form.
...
\begin{itemize}
  \item \textbf{Model scope:} The method is limited to the class of EZ models and their design matrix extensions. While all identifiable models \textit{have} both the forward and inverse equations, they are often not known or mathematically inconvenient.
  \item \textbf{Distributional assumptions:} The method requires that summary statistics have known or approximately known sampling distributions. The normal approximation for mean RT and Gamma approximation for variance are asymptotically valid but may be less accurate for very small samples ($n < 30$) or when RT distributions are highly skewed, but good approximations of sampling distributions are known for very many summary statistics.
  \item \textbf{Independence assumption:} We assume approximate independence between summary statistics. While this holds asymptotically and is approximately valid for sufficiently large samples, correlations may be non-negligible for very small samples or models beyond our consideration.
\end{itemize}
...
\fi
The simulation studies reported in this paper can be reproduced using the C implementation available in the \texttt{ez-bootstrap} GitHub repository (see \texttt{https://github.com/joachimvandekerckhove/ez-bootstrap}). The implementation requires the GNU Scientific Library (GSL) for random number generation and statistical distributions.

To reproduce the studies:

\begin{enumerate}
\item Clone the repository %and navigate to the \texttt{cpp} directory.
\item Install dependencies: GSL development libraries (e.g., \texttt{libgsl-dev} on Debian/Ubuntu systems).
\item Build the executables by running \texttt{make}.
...
The script executes both simulation studies with the following parameters: 10,000 simulation repetitions, sample size $n = 100$ trials per condition, and 1,000 bootstrap samples per repetition. Study 1 evaluates coverage for the single-condition EZ diffusion model; Study 2 evaluates coverage for the design matrix extension with four conditions and three regression weights per parameter type.

The simulation outputs include coverage rates for each parameter (or beta weight) and computational performance metrics. The random seed is set to 100 for reproducibility; this can be modified in the script to generate different random sequences while maintaining reproducibility.

Full output of the simulation script is reproduced below:
% \cleardoublepage
\lstinputlisting[basicstyle=\ttfamily\small,breaklines=true,literate={≈}{{$\approx$}}1]{includes/simulation-output.txt}
\end{document}
