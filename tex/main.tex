...
\begin{equation}
\widehat{\driftangle} - \driftangle \sim \mathcal{N}\left(0, \frac{1}{N\kappa R}\right) \nonumber
\end{equation}
where $R = I_1(\kappa)/I_0(\kappa)$. The variance decreases rapidly with concentration $\kappa$, making estimation most difficult when drift magnitude or boundary radius are small.

For the circular variance ($\VCA{}$), the mean resultant length $R = \sqrt{\bar{c}^2 + \bar{s}^2}$ is approximately normal for moderate-to-large $N$, with mean $R$ and variance:
\begin{equation}
\operatorname{Var}(R) \approx \frac{1 - R^2}{N} \nonumber
\end{equation}
