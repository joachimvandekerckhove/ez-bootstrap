...
For the circular mean of choice angles (MCA), the angular error $\widehat{\driftangle} - \driftangle$ is approximately normally distributed for large $N$, and so:
\begin{equation}
...
For the circular variance ($\VCA{}$), the mean resultant length $L = \sqrt{\bar{c}^2 + \bar{s}^2}$ is approximately normal for moderate-to-large $N$, where $\bar{c} = \frac{1}{N}\sum_{i=1}^{N} \cos(\theta_i)$ and $\bar{s} = \frac{1}{N}\sum_{i=1}^{N} \sin(\theta_i)$ are the mean cosine and mean sine of the observed angles, respectively. The mean resultant length $L$ has mean $R$ and variance:
\begin{equation}
\operatorname{Var}(L) \approx \frac{1 - R^2}{N} \nonumber
\end{equation}
Consequently, $\widehat{\VCA{}} = 1 - R$ is also asymptotically normal with the same variance. The circular variance is only weakly sensitive to outliers in angle (since angles wrap on the circle). However, parameter estimation becomes unstable when $R$ is near zero because: (1)~the variance of the estimator $\widehat{R}$ approaches $1/N$ (its maximum), making $\widehat{R}$ imprecise; (2)~when $R \approx 0$, the data are essentially uniform on the circle, providing little information about the concentration parameter $\kappa$; and (3)~the inverse transformation from $R$ to $\kappa$ (and subsequently to drift magnitude and boundary radius) becomes numerically sensitive to small errors in $\widehat{R}$.
...
\end{equation}
where $m_4$ is the fourth central moment of the RT distribution. This statistic is the most contamination-sensitive in EZ-CDM, as a small number of extreme RTs can cause large upward shifts (see \citeNP{chávez_de_la_peña_etal:preprint:hierarchical}, for a possible treatment using robust summary statistics).
