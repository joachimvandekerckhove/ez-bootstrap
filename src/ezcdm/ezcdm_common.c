...
double solve_kappa_from_R(double R) {
    ...
    if (R < 1e-6) {
        /* For very small R: Use Taylor expansion
         * For small kappa, R ≈ kappa/2, so kappa ≈ 2R
         * This is the leading term of the expansion
         */
        kappa = 2.0 * R;
    } else if (R < 0.53) {
        /* For R < 0.53: kappa ≈ 2R + R^3 + (5/6)R^5 */
        kappa = 2.0 * R + R * R * R + (5.0 / 6.0) * R * R * R * R * R;
    } else if (R < 0.85) {
        /* For 0.53 <= R < 0.85: kappa ≈ -0.4 + 1.39R + 0.43/(1-R) */
        kappa = -0.4 + 1.39 * R + 0.43 / (1.0 - R);
    } else {
        /* For R >= 0.85: kappa ≈ 1/(R^3 - 4R^2 + 3R) */
...
CDMParameters ezcdm_inverse(CDMSummaryStats stats) {
    ...
    /* Step 4: Solve for drift_mag using correct inverse equation
     * From forward: VRT = (r^2/μ^2) R^2 + (2r/μ^3) R - r^2
     * where r = boundary_radius, μ = drift_mag, κ = rμ
     * 
     * Substituting r = κ/μ and solving the quadratic in μ^2:
     * μ^2 = (-κ^2 + sqrt(κ^4 + 4*VRT*(κ^2*R^2 + 2*κ*R))) / (2*VRT)
     */
    if (vrt <= 0.0) {
        return params;
    }
    ...
    double kappa_4 = kappa_sq * kappa_sq;
    double R_sq = R * R;
    
    /* Compute the discriminant: κ^4 + 4*VRT*(κ^2*R^2 + 2*κ*R) */
    double discriminant_term = kappa_sq * R_sq + 2.0 * kappa * R;
    double discriminant = kappa_4 + 4.0 * vrt * discriminant_term;
    
    if (discriminant < 0.0) {
        return params;  /* Invalid discriminant */
    }
    
    double sqrt_discriminant = sqrt(discriminant);
    
    /* μ^2 = (-κ^2 + sqrt(discriminant)) / (2*VRT) */
    double drift_mag_sq = (-kappa_sq + sqrt_discriminant) / (2.0 * vrt);
    
    if (drift_mag_sq <= 0.0) {
        return params;  /* Invalid drift_mag^2 */
    }
    
    params.drift_mag = sqrt(drift_mag_sq);
